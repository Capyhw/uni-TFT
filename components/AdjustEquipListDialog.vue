	...
	<tft-comm-dialog ref="wrap">
		<view class="dialog-container dialog-select-equipment" v-if="equip_list">
			<view class="index-tab-title">
				<view class="title-item" :class="select_class === 'normal' && 'selected'"
					@click="selectClass('normal')">
					<view class="inner">
						<view>正常装备</view>
					</view>
				</view>
				<view class="title-item" :class="select_class === 'job' && 'selected'" @click="selectClass('job')">
					<view class="inner">
						<view>转职纹章</view>
					</view>
				</view>
			...
			<view class="list">
				<template v-for="equip in render_equip_list">
					<view :key="equip.data.TFTID" class="item"
						:style="{'background-image':'url('+equip.data.imagePath+')'}"
		...
		computed: {
			render_equip_list() {
				...
				_.forEach(this.equip_data_all, equip => {
					if (this.select_class === 'normal') {
						if (!equip.formula) return;
						if (+equip.jobId || +equip.raceId) return;
					} else if (this.select_class === 'job') {
						if (!+equip.jobId && !+equip.raceId) return;
					}
					var rs = {
						amount: 0,
				...
				return _.orderBy(equipList, item => {
					return +item.data.jobId || +item.data.raceId;
				}, 'desc');
			}
		},
		methods: {
...
<style>
	...
	.dialog-select-equipment .index-tab-title>.title-item>.inner:after {
		content: "";
		display: block;
		width: 0;
		height: 7px;
	...
	.dialog-select-equipment .index-tab-title>.title-item.selected>.inner::after {
		width: 46px;
	}
